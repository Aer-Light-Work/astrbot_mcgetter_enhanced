...
def setup_astrbot_command_mock() -> None:
    """注册足以导入并直接调用 main.py 命令 handler 的 AstrBot mock。"""
...
async def test_command_handlers() -> None:
    """验证文档列出的管理、显示和趋势命令能执行并正确保存状态。"""
    with tempfile.TemporaryDirectory() as temp_dir:
        json_path = Path(temp_dir) / "command-test.json"
        ...
        for command in ("/mc", "/mcadd", "/mcget", "/mcdel", "/mcup", "/mclist", "/mccleanup", "/mcdata", "/mcpreset", "/mcnote", "/mcalias", "/mctoggle"):
        ...
        original_get_all_trend_histories = plugin_main.get_all_trend_histories
        original_get_server_status = plugin_main.get_server_status
        original_bar_chart = plugin_main.generate_bar_chart_image
        try:
            observed = {}
            ...
            plugin_main.get_all_trend_histories = get_all_trend_histories
            plugin_main.get_server_status = get_server_status
            plugin_main.generate_bar_chart_image = lambda *_args, **_kwargs: "chart"
            assert await collect(plugin.mcdata(event, "48")) == [["chart"]]
            assert observed["hours"] == 48, "/mcdata 48 应作为全服 48 小时处理"
        finally:
            plugin_main.get_all_trend_histories = original_get_all_trend_histories
            plugin_main.get_server_status = original_get_server_status
            plugin_main.generate_bar_chart_image = original_bar_chart
        ...
        original_auto_cleanup_servers = plugin_main.auto_cleanup_servers
        try:
            async def auto_cleanup_servers(_path: Path):
                return []

            plugin_main.auto_cleanup_servers = auto_cleanup_servers
            assert await collect(plugin.mccleanup(event)) == ["没有需要清理的服务器"]
        finally:
            plugin_main.auto_cleanup_servers = original_auto_cleanup_servers
        ...
        try:
            ...
            plugin_main.get_server_status = fail_if_called
...
async def main() -> None:
    test_command_documentation_consistency()
    await test_command_handlers()
    print("命令 handler 回归测试通过！✓")
